                                ...
                                @for($w=1; $w<5; $w++)
                                    ...
                                    <div id="accordion{{$w}}">
                                        <div class="card">
                                            ...
                                            <div id="collapse{{$w}}" class="collapse show" aria-labelledby="heading{{$w}}" data-parent="#accordion{{$w}}">
                                                <div class="card-body">
                                                    @for($q=1;$q<8;$q++)
                                                        <label class="col-12">
                                                            السؤال {{$q}} | الإسبوع {{ $w }}
                                                            <input type="text" name="question[{{$w}}][{{$q}}]" value="{{ count($questions) > 0 ? $questions[$question]->question : ''  }}" class="form-control" required>
                                                        </label>
                                                        جواب السؤال {{$q}}
                                                        @for($i = 1 ; $i<5; $i++)
                                                            <div class="form-check mt-1">
                                                                <label class="form-check-label col-10">
                                                                    <input class="form-check-input" type="radio" value="{{ $i }}"
                                                                           name="correct[{{$w}}][{{$q}}]" required {{ count($questions) > 0 ? ($questions[$question]->correct_answer == $questions[$question]->answers[$i-1]->id ?'checked' : '') : '' }}>
                                                                    <input type="text" value="{{ $questions[$question]->answers[$i-1]->answer }}" name="answer[{{$w}}][{{$q}}][{{ $i }}]" class="form-control">
                                                                </label>
                                                            </div>
                                                        @endfor
                                                        @php $question++;@endphp
                                                    ...
                                                    <label class="mt-3">
                                                        تاريخ نشر أسئلة {{$w}}:
                                                        <input type="date" name="date[{{$w}}]" value="{{ $questions[$question-1]->publish_date }}"
